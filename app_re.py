...
# =========================================================
# 🧠 メインロジック (Execution) - [Phase 3: 全面刷新とエンジン接続]
# =========================================================
if run_btn:
    if not input_weights_dict:
        st.error("ポートフォリオを入力してください")
        ...
        with st.spinner("🔍 市場構造を分析中... (Applying 3-Factor Model & t-Distribution)"):
            try:
                ...
                # 2. 統計指標とファクター分析の実行 (AdvancedStats & FactorAnalyzer)
                metrics = AdvancedStats.calculate_metrics(returns)
                metrics['annual_return'] = returns.mean() * 12
                metrics['volatility'] = returns.std() * np.sqrt(12)
                ...
                # 3. シナリオ生成と未来予測 (StochasticScenarioGenerator & ProjectionCore)
                start_date = target.index[0]
                waves = StochasticScenarioGenerator.generate_3factor_waves(
                    start_date=start_date, n_sims=n_sims, horizon_months=months
                )
...
if st.session_state.audit_result is not None:
    ...
    with t1:
        ...
        fig_fan.update_layout(title="Portfolio Value Projection (Monte Carlo)", xaxis_title="Months Ahead", yaxis_title="Portfolio Value", template="plotly_dark", height=500)
        st.plotly_chart(fig_fan, use_container_width=True)
    ...
    with t2:
        ...
        with c2:
            ...
            fig_rec.add_vline(x=12, line_dash="dash", line_color="yellow", annotation_text="1 Year")
            fig_rec.add_vline(x=36, line_dash="dash", line_color="red", annotation_text="3 Years")
            fig_rec.update_layout(template="plotly_dark", bargap=0.1, xaxis_title="Months to Recover Principal")
            st.plotly_chart(fig_rec, use_container_width=True)
    ...
    with t3:
        ...
        pain_idx = abs(metrics.get('max_dd', 0)) * 100
        c2.metric("Pain Index", f"{pain_idx:.1f}", help="投資家が感じる痛みの平均値（最大下落率より算出）。")
